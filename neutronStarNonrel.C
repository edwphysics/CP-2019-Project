...
//NEUTRON STAR - Nonrelativistic Case gamma=5/3
//y = pressure, z = mass

/*
	COMMENTS: 
	ADDITIONS: GR Factors, R0, k, Gamma, R rescaling, slide Mass to avoid divergence
	ISSUES: 
*/

//GLOBAL CONSTANTS AND VARIABLES
int n = 8000, l = 1;
double alpha = 1, beta = 0.7636, k = 1.914; //DE Constants
double Delta = 1e-30;			 			//Sliding constant to avoid r=0 divergence
double Gamma = 5./3.;						//Polytripe parameter
double R0 = 1.47;							//one-half of Schwartzschild radius
...
double f(double y[], double z[], double t, int j){
	...
	if (flag == false)
	{
		printf("%e\t", (alpha*pow(y[0],1./Gamma)*z[0]/pow(t,2)));
		printf("%e\t", (1+pow(k,1./Gamma)*pow(y[0],1-1./Gamma)));
		printf("%e\t", (1+beta*pow(k,1./Gamma)*pow(t,2)*y[0]/z[0]));
		printf("%e\n", (1-2*R0*z[0]/t));
	}
	v[0] = -(alpha*pow(y[0],1./Gamma)*z[0]/pow(t,2))*(1+pow(k,1./Gamma)*pow(y[0],1-1./Gamma))*(1+beta*pow(k,1./Gamma)*pow(t,2)*y[0]/z[0])/(1-2*R0*z[0]/t);
	return v[j];
}
...
double g(double y[], double z[], double t, int j){
	//GENERALIZED VELOCITY
	double v[l];
	v[0] = beta*pow(t,2)*pow(y[0],1./Gamma);
	return v[j];
}
...
void principal(double p0){
	...
	c1->Divide(2,1);
	c1->cd(1);
	TGraphErrors *g1 = new TGraphErrors(n, T, y1);
	...
	g1->Draw("al");
	c1->cd(2);
	TGraphErrors *g2 = new TGraphErrors(n, T, z1);
...
void neutronStarNonrelV1(){
	double p0 = 1e-4; //Initial Pressure
