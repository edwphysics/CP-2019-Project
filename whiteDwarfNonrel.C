...
//WHITE DWARF - Nonrelativistic Case gamma=5/3
//y = pressure, z = mass

/*
	COMMENTS: Evaluate for p0=1e-16 and 1e-15, shows the result. 
	ADDITIONS: Criterion of 5-order, so p0 gives criterion value and Gamma parameter
	ISSUES: 
*/

//GLOBAL CONSTANTS AND VARIABLES
int n = 8000, l = 1;
double alpha = 0.05, beta = 0.005924; //DE Constants
...
double f(double y[], double z[], double t, int j){
	...
	double v[l];
	v[0] = -alpha*pow(y[0],1./Gamma)*z[0]/pow(t,2);
	return v[j];
}
...
void whiteDwarfNonrelV2(){
	double p0 = 1e-15; //Initial Pressure
